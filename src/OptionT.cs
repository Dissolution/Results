...
/// </typeparam>
/// <remarks>
/// <see cref="Option{T}"/> does not protect against <c>null</c> references, <c>Option&lt;string?&gt;.Some(null)</c> is perfectly valid<br/>
/// Use <see cref="NotNull"/> to transform a possibly-<c>null</c> value into an <see cref="Option{T}"/> where <see cref="Some"/> is not-<c>null</c>
/// </remarks>
    ...
    IEnumerable<T>
{
    ...
    [Obsolete($"""
               Do not instantiate a new Option!
               Use {nameof(Some)}(), {nameof(None)}(), or {nameof(NotNull)}().
               """, true)]
    public Option()
    {
        _some = false;
    ...
    {
        return Match(some => some?.GetHashCode() ?? 1, () => 0);
    }

    public override string ToString()
    {
        return Match(some => $"Some<{typeof(T).Name}>({some})", () => "None");
    }
}