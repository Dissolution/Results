...
//    }
    ...
    {
        var aggEx = new AggregateException(exceptions.Where(static ex => ex is not null)!);
    ...
    /// <see cref="Result"/><c>.Error(</c><see cref="AggregateException"/><c>)</c>
    /// </returns>
    public static Result Errors(IEnumerable<Exception?> exceptions)
    {
        var aggEx = new AggregateException(exceptions.Where(static ex => ex is not null)!);
        return new Result(false, aggEx);
    }
    ...
    /// The <see cref="Type"/> of value to attach to an <see cref="Ok"/> <see cref="Result"/>
    /// </typeparam>
    /// <param name="value">
    /// The value to attach to the <see cref="Result{TValue}"/> if this is an <see cref="Ok"/> <see cref="Result"/>
    /// </param>
    /// <returns>
    /// <see cref="Result{TValue}"/>.Ok(<paramref name="value"/>) if this is <see cref="Ok"/><br/>
    /// <see cref="Result{TValue}"/>.Error(<see cref="Exception"/>) if this is <see cref="Error"/>
    /// </returns>
